      ...
      <div className="max-w-4xl mx-auto px-4 py-10">
        ...
        <div className="mb-10">
          <div className="flex items-center gap-3 mb-3">
            <div className="w-10 h-10 rounded-xl bg-raised border border-muted flex items-center justify-center">
              <Zap size={20} className="text-primary" />
            </div>
            <div>
              <h1 className="font-heading text-3xl font-bold text-primary">AutoPost AI</h1>
              <p className="text-sm text-secondary">3-Stage Gemini Validation Pipeline</p>
            </div>
          </div>

          <div className="grid grid-cols-3 gap-3 mt-6">
            {['Stage 1: Deep Draft', 'Stage 2: Fact-Check', 'Stage 3: Polish'].map((s, i) => (
              <div key={i} className="text-center rounded-xl border border-border bg-surface p-3">
        ...
        <div className="rounded-2xl border border-border bg-surface p-6 mb-6">
          <h2 className="text-sm font-semibold text-primary mb-1 flex items-center gap-2">
            <Key size={15} className="text-secondary" />
            Gemini API Key
          </h2>
          <p className="text-xs text-secondary mb-4">
            Get your free key at <a href="https://aistudio.google.com/apikey" target="_blank" rel="noopener noreferrer" className="text-primary hover:underline">aistudio.google.com</a>. Stored locally in your browser only.
          </p>
          ...
          <div className="flex gap-3">
            <div className="relative flex-1">
              <input
                ...
                placeholder="AIza..."
                className="w-full bg-canvas border border-border rounded-xl px-4 py-3 text-primary text-sm outline-none focus:border-primary/60 transition-colors pr-10 font-mono placeholder-secondary/50"
              />
              <button
                type="button"
            ...
            <button
              onClick={saveKey}
              className={`px-5 py-3 rounded-xl text-sm font-medium transition-all ${keySaved ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30' : 'bg-primary text-canvas hover:bg-white'}`}
            >
              {keySaved ? <><CheckCircle size={14} className="inline mr-1.5" />Saved</> : 'Save Key'}
            </button>
          </div>
        </div>
        ...
        {/* Pipeline Inputs */}
        <div className="rounded-2xl border border-border bg-surface p-6 mb-6">
          <h2 className="text-sm font-semibold text-primary mb-5">Article Configuration</h2>
          ...
          <div>
            ...
            <div className="flex gap-2 mb-3">
              <input
                ...
                className="flex-1 bg-canvas border border-border rounded-xl px-4 py-2.5 text-primary text-sm outline-none focus:border-primary/60 transition-colors placeholder-secondary/50"
              />
              <button
                onClick={addKeyword}
                className="px-4 py-2.5 bg-raised hover:bg-muted rounded-xl text-primary transition-colors"
              >
                <Plus size={16} />
              </button>
            </div>
            {keywords.length > 0 && (
              <div className="flex flex-wrap gap-2">
                {keywords.map(kw => (
                  <span key={kw} className="flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-full bg-raised text-secondary">
                    {kw}
          ...
          {/* Info */}
          <div className="mt-5 flex items-start gap-2.5 bg-canvas rounded-xl p-4 border border-border">
        ...
        {!running && !result && (
          <button
            ...
            className="w-full flex items-center justify-center gap-3 bg-primary hover:bg-white disabled:opacity-40 disabled:cursor-not-allowed text-canvas font-bold py-4 rounded-2xl transition-all duration-200 text-base"
          >
            <Zap size={20} />
        ...
        {(running || stages.length > 0) && (
          <div className="rounded-2xl border border-border bg-surface p-6 mb-6">
            <h2 className="text-sm font-semibold text-primary mb-5 flex items-center gap-2">
              {running ? (
                <><div className="w-3 h-3 rounded-full bg-primary animate-pulse" />Generating{running && stages.some(s => s.message?.includes('retrying')) ? ' (retrying…)' : '…'}</>
              ) : stages.some(s => s.status === 'error') ? (
                <><AlertTriangle size={14} className="text-red-400" />Pipeline Failed</>
              ) : (
              ...
              {stages.map((stage, i) => (
                <div key={i} className="flex items-center gap-4">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm transition-all ${
                    ...
                    {stage.status === 'done' ? '✓'
                     : stage.status === 'running' ? <Clock size={14} className="animate-spin" />
                     : stage.status === 'error' ? '✗'
                     : i + 1}
                  </div>
                  <div className="flex-1">
                    ...
                    {stage.message && (
                      <p className="text-xs text-secondary mt-0.5">{stage.message}</p>
                    )}
                  </div>
        ...
        {result && !running && (
          <div className="space-y-4">
            ...
            {result.audit && (
              <div className="rounded-2xl border border-border bg-surface overflow-hidden">
                <button
                  ...
                  className="w-full flex items-center justify-between p-5 text-left hover:bg-raised/30 transition-colors"
                >
                  <span className="text-sm font-semibold text-primary">
                    🔍 Audit Report — {result.audit.vulnerabilities.length} issues flagged
                  </span>
                  {auditOpen ? <ChevronUp size={16} className="text-secondary" /> : <ChevronDown size={16} className="text-secondary" />}
                </button>
                {auditOpen && (
                  <div className="px-5 pb-5 space-y-4 border-t border-border">
                    {result.audit.vulnerabilities.length > 0 && (
                      <div>
                        <p className="text-xs font-semibold text-red-400 mb-2 uppercase tracking-wider mt-4">Flagged Issues</p>
                        <ul className="space-y-2">
                          {result.audit.vulnerabilities.map((v, i) => (
                            <li key={i} className="text-xs text-secondary bg-red-950/20 border border-red-900/30 rounded-lg p-3">
                              {v}
                            </li>
                          ))}
                        </ul>
                    ...
                    {result.audit.suggestions.length > 0 && (
                      <div>
                        <p className="text-xs font-semibold text-emerald-400 mb-2 uppercase tracking-wider">Applied Suggestions</p>
                        <ul className="space-y-2">
                          {result.audit.suggestions.map((s, i) => (
                            <li key={i} className="text-xs text-secondary bg-emerald-950/20 border border-emerald-900/30 rounded-lg p-3">
                              {s}
                            </li>
                          ))}
                        </ul>
            ...
            {result.outline && (
              <div className="rounded-2xl border border-border bg-surface overflow-hidden">
                <button
                  ...
                  className="w-full flex items-center justify-between p-5 text-left hover:bg-raised/30 transition-colors"
                >
                  <span className="text-sm font-semibold text-primary">🏗️ Article Outline</span>
                  {outlineOpen ? <ChevronUp size={16} className="text-secondary" /> : <ChevronDown size={16} className="text-secondary" />}
                </button>
                {outlineOpen && (
                  <div className="px-5 pb-5 border-t border-border">
                    <pre className="text-xs text-secondary whitespace-pre-wrap leading-relaxed mt-4 font-sans">
                      {result.outline}
                    </pre>
                  </div>
                )}
            ...
            <div className="rounded-2xl border border-border bg-surface overflow-hidden">
              ...
              >
                <span className="text-sm font-semibold text-primary">
                  ✨ Article Preview — {result.content.split(/\s+/).length.toLocaleString()} words
                </span>
                {previewOpen ? <ChevronUp size={16} className="text-secondary" /> : <ChevronDown size={16} className="text-secondary" />}
              </button>
              {previewOpen && (
                <div className="px-5 pb-5 border-t border-border">
                  <div
                    className="prose-premium mt-5"
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              {result.status === 'ready_to_publish' && (
                <button
                  onClick={publishPost}
                  className="flex-1 flex items-center justify-center gap-2 bg-primary hover:bg-white text-canvas font-bold py-4 rounded-2xl transition-all duration-200"
                >
                  <Send size={18} />
                  Publish Now
                </button>
              ...
              <button
                onClick={saveDraft}
                className="flex-1 flex items-center justify-center gap-2 bg-surface border border-border hover:border-primary/50 text-primary font-semibold py-4 rounded-2xl transition-all duration-200"
              >
                <Save size={18} />
              ...
              <button
                ...
                className="flex items-center justify-center gap-2 bg-surface border border-border hover:border-red-400/50 text-secondary hover:text-red-400 px-6 py-4 rounded-2xl transition-all duration-200"
              >
                <X size={18} />
                Reset
              </button>
